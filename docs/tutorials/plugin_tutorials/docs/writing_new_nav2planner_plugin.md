...
| createPlan()         | Method is called when planner server demands a global plan for specified start pose, goal pose, and intermediate viapoint poses. This method returns *nav_msgs\:\:msg\:\:Path* carrying global plan. This method takes 4 input params: start pose, goal pose, a vector of intermediate poses and a function to check if the action has been canceled. | Yes                      |
